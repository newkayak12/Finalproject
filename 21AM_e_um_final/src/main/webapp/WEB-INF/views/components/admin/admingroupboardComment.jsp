		...
		<tr>
			...
			<c:if test="${list.size()>0}">
				...
				<tr>
					...
					<td>
						<c:choose>
							<c:when test="${i.groupCommentBlock =='unblock' }">
	...
	<script type="text/javascript">
		...
		function feedBlocker(seq){
			...
			if($("#status"+seq).html() == '게시'){
				/* 숨김 */
				$.ajax({
					url:"${pageContext.request.contextPath}/admin/groupboardcommentblock",
					data:{"seq":seq},
					success:data=>{
						if(data>0){
							console.log(data)
				...
				$.ajax({
					url:"${pageContext.request.contextPath}/admin/groupboardcommentunblock",
					data:{"seq":seq},
					success:data=>{
						if(data>0){
							console.log(data)
