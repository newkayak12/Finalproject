...
//		profileId="'"+profileId+"'";
	...
	public static String getPageBarDef(int totalData, int cPage, int numPerPage, String url, String functionName) {
		int totalPage=(int)Math.ceil((double)totalData/numPerPage);
		int pageBarSize=5;
		int pageNo=((cPage-1)/pageBarSize)*pageBarSize+1;
		int pageEnd=pageNo+pageBarSize-1;
//		profileId="'"+profileId+"'";
		...
		pageBar +="<script>function "+functionName+"(cPage){$.ajax({url:'"+url+"',data:{'cPage': cPage},success:data=>{$('#quesTable').html(data);}})}";
